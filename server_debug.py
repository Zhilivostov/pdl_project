...
class TcpServerWindow(QMainWindow):
    ...
    def device_loop_iteration(self):
        ...
        self.stop_work()
        stop_work_time = (time.perf_counter() - started_at) * 1000
        print(f"stop_work {stop_work_time:.2f} mc")
        self.write_registers_value()
        write_registers_time = (time.perf_counter() - started_at) * 1000
        print(f"writing_registers {write_registers_time:.2f} mc")
        #self.read_register()
        self.start_work()
        work_time = (time.perf_counter() - started_at) * 1000
        print(f"work_time {work_time:.2f} mc")
        self.read_data()
        read_data_time = (time.perf_counter() - started_at) * 1000
        print(f"read_data {read_data_time:.2f} mc")
