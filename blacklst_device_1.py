import sys
from PyQt6.QtCore import QObject, QTimer, QFile, QTextStream, QStringConverter
...
class Server(QObject):
    ...
    def readPendingDatagrams(self):
        ...
        while self.device_socket.hasPendingDatagrams():
            # Рекомендуется использовать receiveDatagram, он содержит адрес отправителя
            datagram = self.device_socket.receiveDatagram()
            ...
            #line = f"[{sender_ip}:{sender_port}] {data}\n"
            #self.stream << line
            ...
            #bytes_list = [f"{b:02X}" for b in data.data()]
            #numbers_string = " ".join(bytes_list)
            
            self.stream << f"{''.join(result)}\n"
            self.stream.flush()
            
            #print(f"Получено от {sender_ip}:{sender_port}: {text}")
            print(f"Получено от {sender_ip}:{sender_port}: {data}")
            #print(f"Получено от {sender_ip}:{sender_port}:")
            #self.print_raw_bytes(data)

        #self.file.close() #закрытие файла
        #print("File closed")
    
    def byte_to_str(bytes_str):
        try:
            return r"\x" + r"\x".join("{b:02x}".format(c) for c in bytes_str)
        except:
            try: #дописать нужно!!!
    ...
    def read_data(self):
        ...
        self.device_socket.write(READ_ALL_DATA_COMMAND)
        
        data, address = self._recv_udp(4096)
        self.check_ack(data)
        #Подключаем сигнал: при получении данных вызывается функция
        self.device_socket.readyRead.connect(self.readPendingDatagrams)

        self.file = QFile("received_data.text")
        #self.file = open("received_data_python.txt", "wb")
        #if not self.file.open(QFile.OpenModeFlag.WriteOnly | QFile.OpenModeFlag.Append | QFile.OpenModeFlag.Text):
        if not self.file.open(QFile.OpenModeFlag.WriteOnly | QFile.OpenModeFlag.Append):
